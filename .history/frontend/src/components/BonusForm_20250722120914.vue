<template>
    <form @submit.prevent="submit" class="space-y-2 border p-4 rounded">
        ...
        <div v-for="(row, idx) in rows" :key="idx" class="flex gap-2 items-center">
            ...
            <button type="button" @click="remove(idx)" class="bg-gray-300 px-2 py-0.5 rounded">+行</button>
        </div>
        <button type="button" @click="add" class="bg-gray-300 px-2 py-0.5 rounded"></button>
    </form>
</template>