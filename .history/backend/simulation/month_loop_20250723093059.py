...
def simulate(params: "SimParams", members_template):
    ...
    for m in range(1, params.months + 1):
        ...
        cum_invited_child += params.invite_per_month  # 仕様: 親と同じ
        cum_invited_grand += params.invite_per_month  # 仕様: 親と同じ

        # 加入人数
        new_child = calc_joins(cum_invited_child, joined_child)
        new_grand = calc_joins(cum_invited_grand, joined_grand)

        # 人数更新
        count_child = next_counts(count_child, new_child, m, params.cont_rate, params.grace_months)
        joined_child += new_child

        count_grand = next_counts(count_grand, new_grand, m, params.cont_rate, params.grace_months)
