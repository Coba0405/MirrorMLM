...
def test_cont_rate_decreases_children():
    ...
    # 月毎のアクティブ子の人数をチェック
    assert results[0]["count_child"] <= 10
    assert results[1]["count_child"] <= results[0]["count_child"]
    assert results[2]["count_child"] <= results[1]["count_child"]
