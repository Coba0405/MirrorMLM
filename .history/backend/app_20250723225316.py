...
def bonus():
    ...
    result = calc_bonus(purchases, initial_members, root_id="A")
...
def distribute():
    ...
    result = distribute_pv(purchaser, total_pv, initial_members, max_depth=2)
...
def run_sim():
    p = request.json
    params = SimParams(**p)
    recs = simulate(params, initial_members)
    return jsonify(recs)
